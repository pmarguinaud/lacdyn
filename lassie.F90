...
CALL SIGAM(PSTACK, KPSTSZ, KPSTPT, KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0L,&
          &PGMV(1,1,YDGMV%YT0%MTL),PGMVS(1,YDGMV%YT0%MSPL))
CALL SIGAM(PSTACK, KPSTSZ, KPSTPT, KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0M,&
...
IF (.NOT.LTWOTL) THEN
  ...
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(PSTACK, KPSTSZ, KPSTPT, KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9L,&
            &PGMV(1,1,YDGMV%YT9%MTL),PGMVS(1,YDGMV%YT9%MSPL))
  CALL SIGAM(PSTACK, KPSTSZ, KPSTPT, KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9M,&
...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    ENDDO
  ELSE
    CALL GPRCP(PSTACK, KPSTSZ, KPSTPT, KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JLON=KIDIA,KFDIA
        PTOD0(JLON,JLEV)=RD*PTOD0(JLON,JLEV)/ZR9(JLON,JLEV)
