...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    CALL GPRCP(KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,ISTPT,KSTSZ,PSTACK,PGFL=PGFL,PR=ZR9)  
    ...
    ENDDO
  ELSE
    CALL GPRCP(KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,ISTPT,KSTSZ,PSTACK,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JLON=KIDIA,KFDIA
        PTOD0(JLON,JLEV)=RD*PTOD0(JLON,JLEV)/ZR9(JLON,JLEV)
