...
CALL SITNU(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,&
          &PGMV(1,1,YDGMV%YT0%MDIV),PTOD0,PSDIV0,KSTPT,KSTSZ,PSTACK)
!   - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
CALL SIGAM(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0L,&
          &PGMV(1,1,YDGMV%YT0%MTL),PGMVS(1,YDGMV%YT0%MSPL),KSTPT,KSTSZ,PSTACK)
CALL SIGAM(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0M,&
          &PGMV(1,1,YDGMV%YT0%MTM),PGMVS(1,YDGMV%YT0%MSPM),KSTPT,KSTSZ,PSTACK)
...
IF (.NOT.LTWOTL) THEN
  ...
  !   and Tau*D (SI term for temperature equation).
  CALL SITNU(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,&
           &PGMV(1,1,YDGMV%YT9%MDIV),PTOD9,PSDIV9,KSTPT,KSTSZ,PSTACK)
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9L,&
            &PGMV(1,1,YDGMV%YT9%MTL),PGMVS(1,YDGMV%YT9%MSPL),KSTPT,KSTSZ,PSTACK)
  CALL SIGAM(KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9M,&
            &PGMV(1,1,YDGMV%YT9%MTM),PGMVS(1,YDGMV%YT9%MSPM),KSTPT,KSTSZ,PSTACK)
ENDIF
...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    CALL GPRCP(KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,KSTPT,KSTSZ,PSTACK,PGFL=PGFL,PR=ZR9)  
    ...
    ENDDO
  ELSE
    CALL GPRCP(KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,KSTPT,KSTSZ,PSTACK,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JLON=KIDIA,KFDIA
        PTOD0(JLON,JLEV)=RD*PTOD0(JLON,JLEV)/ZR9(JLON,JLEV)
