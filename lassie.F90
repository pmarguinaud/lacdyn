...
CALL SITNU(KIDIA,KFDIA,YRDIM%NPROMA,YRDIMV%NFLEVG,&
          &PGMV(1,1,YDGMV%YT0%MDIV),PTOD0,PSDIV0)
!   - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
CALL SIGAM(KIDIA,KFDIA,YRDIM%NPROMA,YRDIMV%NFLEVG,PGAGT0L,&
          &PGMV(1,1,YDGMV%YT0%MTL),PGMVS(1,YDGMV%YT0%MSPL))
CALL SIGAM(KIDIA,KFDIA,YRDIM%NPROMA,YRDIMV%NFLEVG,PGAGT0M,&
...
IF (.NOT.LTWOTL) THEN
  ...
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(KIDIA,KFDIA,YRDIM%NPROMA,YRDIMV%NFLEVG,PGAGT9L,&
            &PGMV(1,1,YDGMV%YT9%MTL),PGMVS(1,YDGMV%YT9%MSPL))
  CALL SIGAM(KIDIA,KFDIA,YRDIM%NPROMA,YRDIMV%NFLEVG,PGAGT9M,&
...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    CALL GPRCP(YRDIM%NPROMA,KIDIA,KFDIA,YRDIMV%NFLEVG,PGFL=PGFL,PR=ZR9)  
    ...
    ENDDO
  ELSE
    CALL GPRCP(YRDIM%NPROMA,KIDIA,KFDIA,YRDIMV%NFLEVG,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JROF=KIDIA,KFDIA
        PTOD0(JROF,JLEV)=RD*PTOD0(JROF,JLEV)/ZR9(JROF,JLEV)
