!OCL  NOEVAL
SUBROUTINE LAVENT(YDGMV,KST,KPROF,KSETTLOFF,LD2TLFF1,PVCRS0,PRDLR0,PDTS2,PRDELP,PEVEL,PATND,&
 & PGMV,PB1,PB2,PWRL9)
...
REAL(KIND=JPRB) :: ZWRL0(YRDIM%NPROMA,YRDIMV%NFLEVG)
...
IF(LTWOTL.AND.YRDYN%LSVTSM) THEN
  DO JLEV=1,YRDIMV%NFLEVG
    PB1(KST:KPROF,YRPTRSLB1%MSLB1WRA+JLEV-YRDIMV%NFLSA)=ZWRL0(KST:KPROF,JLEV)
  ENDDO
ENDIF
...
IF (LTWOTL.AND.LELTRA) THEN
  ...
  DO JLEV=1,YRDIMV%NFLEVG
    PB1(KST:KPROF,YRPTRSLB1%MSLB1WR0+JLEV-YRDIMV%NFLSA)=ZWRL0(KST:KPROF,JLEV)
    PB2(KST:KPROF,YRPTRSLB2%MSLB2URL+JLEV-1)=PB1(KST:KPROF,YRPTRSLB1%MSLB1UR0+JLEV-YRDIMV%NFLSA)
    PB2(KST:KPROF,YRPTRSLB2%MSLB2VRL+JLEV-1)=PB1(KST:KPROF,YRPTRSLB1%MSLB1VR0+JLEV-YRDIMV%NFLSA)
    PB2(KST:KPROF,YRPTRSLB2%MSLB2WRL+JLEV-1)=PB1(KST:KPROF,YRPTRSLB1%MSLB1WR0+JLEV-YRDIMV%NFLSA)
  ENDDO

  IF (LD2TLFF1) THEN
    DO JLEV=1,YRDIMV%NFLEVG
      PB1(KST:KPROF,YRPTRSLB1%MSLB1UR9+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MU)
      PB1(KST:KPROF,YRPTRSLB1%MSLB1VR9+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MV)
    ENDDO
    IF (LVERCOR) THEN
      DO JLEV=1,YRDIMV%NFLEVG
        DO JROF=KST,KPROF
...
ELSEIF (LTWOTL.AND.(.NOT.LELTRA)) THEN
  ...
  IF (.NOT.LLCT) THEN
    ...
    IF ( NSTEP <= 0 .OR. LLNESCT ) THEN
      ...
      ELSE
        DO JLEV=1,YRDIMV%NFLEVG
          PB1(KST:KPROF,YRPTRSLB1%MSLB1UR0+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MU)
          PB1(KST:KPROF,YRPTRSLB1%MSLB1VR0+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MV)
        ENDDO
      ENDIF
      DO JLEV=1,YRDIMV%NFLEVG
        PB2(KST:KPROF,YRPTRSLB2%MSLB2URL+JLEV-1)=PB1(KST:KPROF,YRPTRSLB1%MSLB1UR0+JLEV-YRDIMV%NFLSA)
    ...
    ELSEIF (LLSETTLST) THEN
      ...
      ELSE
        DO JLEV=1,YRDIMV%NFLEVG
          PB2(KST:KPROF,YRPTRSLB2%MSLB2URL+JLEV-1)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MU)
          PB2(KST:KPROF,YRPTRSLB2%MSLB2VRL+JLEV-1)=PGMV(KST:KPROF,JLEV,YDGMV%YT0%MV)
        ENDDO
      ENDIF
    ENDIF
    IF (LVERCOR) THEN
      PGMV(KST:KPROF,1:YRDIMV%NFLEVG,YDGMV%YT9%MVCASRS)=PVCRS0(KST:KPROF,1:YRDIMV%NFLEVG,YYTVC0%M_VCASRSF)
    ELSEIF (LRWSDLR) THEN
      PGMV(KST:KPROF,1:YRDIMV%NFLEVG,YDGMV%YT9%MRDLASR)=PRDLR0(KST:KPROF,1:YRDIMV%NFLEVG,JP_ASRF)
    ENDIF
    ...
    IF ( NSTEP <= 0 .OR. LLNESCV ) THEN
      ...
      DO JLEV=1,YRDIMV%NFLEVG
        ...
        PB2(KST:KPROF,YRPTRSLB2%MSLB2WRL+JLEV-1)=PB1(KST:KPROF,YRPTRSLB1%MSLB1WR0+JLEV-YRDIMV%NFLSA)
      ENDDO
      IF (NSTEP == 0 .AND. YRDYN%LSETTLSVF) PWRL9(KST:KPROF,1:YRDIMV%NFLEVG)=0.0_JPRB
...
! detect noisy gridpoints which should not be extrapolated
        ...
        DO JLEV=YRDYN%NFLEVSF+1,YRDIMV%NFLEVG
          ...
          ENDDO
        ENDDO
        PWRL9(KST:KPROF,1:YRDIMV%NFLEVG)=PGMV(KST:KPROF,1:YRDIMV%NFLEVG,YDGMV%YT9%MEDOT)
      ELSE
        DO JLEV=1,YRDIMV%NFLEVG
          DO JROF=KST,KPROF
            PB1(JROF,YRPTRSLB1%MSLB1WR0+JLEV-YRDIMV%NFLSA)=2.0_JPRB*ZWRL0(JROF,JLEV)&
        ...
        ENDDO
      ENDIF
      PB2(KST:KPROF,YRPTRSLB2%MSLB2WRL:YRPTRSLB2%MSLB2WRL+YRDIMV%NFLEVG-1)=ZWRL0(KST:KPROF,1:YRDIMV%NFLEVG)
    ENDIF
    PGMV(KST:KPROF,1:YRDIMV%NFLEVG,YDGMV%YT9%MEDOT)=ZWRL0(KST:KPROF,1:YRDIMV%NFLEVG)
  ...
  ELSE 
    ...
    ELSE
      DO JLEV=1,YRDIMV%NFLEVG
        PB1(KST:KPROF,YRPTRSLB1%MSLB1UR0+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT9%MU)
        PB1(KST:KPROF,YRPTRSLB1%MSLB1VR0+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT9%MV)
      ENDDO
    ENDIF
    DO JLEV=1,YRDIMV%NFLEVG
      PB1(KST:KPROF,YRPTRSLB1%MSLB1WR0+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT9%MEDOT)
    ENDDO
    ...
    ENDIF
    DO JLEV=1,YRDIMV%NFLEVG
      PB2(KST:KPROF,YRPTRSLB2%MSLB2WRL+JLEV-1)=ZWRL0(KST:KPROF,JLEV)
    ENDDO
...
ELSE
  ...
  IF (LD2TLFF1) THEN
    DO JLEV=1,YRDIMV%NFLEVG
      PB1(KST:KPROF,YRPTRSLB1%MSLB1UR9+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT9%MU)
      PB1(KST:KPROF,YRPTRSLB1%MSLB1VR9+JLEV-YRDIMV%NFLSA)=PGMV(KST:KPROF,JLEV,YDGMV%YT9%MV)
    ENDDO
    IF (LVERCOR) THEN
      DO JLEV=1,YRDIMV%NFLEVG
        DO JROF=KST,KPROF
