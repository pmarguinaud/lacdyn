...
!      1.04  Type TVERCOR: pointers for output of GPVC.. routines (option LVERCOR=T).

TYPE TVERCOR
INTEGER(KIND=JPIM) :: M_VCRSSAF     ! "rs/a" at full levels
INTEGER(KIND=JPIM) :: M_VCRSSAH     ! "rs/a" at half levels
INTEGER(KIND=JPIM) :: M_VCASRSF     ! "a/rs" at full levels
INTEGER(KIND=JPIM) :: M_VCASRSH     ! "a/rs" at half levels
INTEGER(KIND=JPIM) :: M_VCRSSAFL    ! zonal derivative of "rs/a" at full levels
INTEGER(KIND=JPIM) :: M_VCRSSAFM    ! meridian derivative of "rs/a" at full levels
INTEGER(KIND=JPIM) :: NDIM          ! total number of fields allocated
END TYPE TVERCOR
...
INTEGER(KIND=JPIM) :: M_DIVDP       ! thin layer eqns: grad(vec(V) * (Delta prehyd))
                                    ! lvercor=T: "grad((rs/a)*vec(V)*(Delta prehyd))"
                                    ! lrwsdlr=T: "(a/r) grad((a/r)*vec(V)*(Delta presim))"
                                    !  "grad" is the semi-reduced gradient ((r/a)nabla).
INTEGER(KIND=JPIM) :: NDIM          ! total number of fields allocated
END TYPE TCTY
...
INTEGER(KIND=JPIM) :: M_TNDASRU     ! tendency for (a/r)*U-wind equation
INTEGER(KIND=JPIM) :: M_TNDASRV     ! tendency for (a/r)*V-wind equation
...
!$acc declare create (YYTXYB0         )
!$acc declare create (YYTXYB5         )
!$acc declare create (YYTXYB9         )
!$acc declare create (YYTXYB95        )
!$acc declare create (YYTXYB0_PHY     )
!$acc declare create (YYTXYB9_PHY     )
!$acc declare create (YYTXYBPP        )
!$acc declare create (YYTXYB          )
!$acc declare create (YYTXYBT         )
!$acc declare create (YYTXYBDER0   )
!$acc declare create (YYTXYBDER5   )
!$acc declare create (YYTXYBDERPP  )
!$acc declare create (YYTXYBDER    )
!$acc declare create (YYTXYBDERT   )
!$acc declare create (YYTRCP0   )
!$acc declare create (YYTRCP5   )
!$acc declare create (YYTRCP9   )
!$acc declare create (YYTRCP95  )
!$acc declare create (YYTVC0       )
!$acc declare create (YYTVC1       )
!$acc declare create (YYTVCPP      )
!$acc declare create (YYTVC        )
!$acc declare create (YYTCTY0        )
!$acc declare create (YYTCTY5        )
!$acc declare create (YYTCTYPP       )
!$acc declare create (YYTCTY         )
!$acc declare create (YYTHW0       )
!$acc declare create (YYTHW9       )
!$acc declare create (YYTHW5       )
!$acc declare create (YYTHW95      )
!$acc declare create (YYTHWPP      )
!$acc declare create (YYTHW        )
!$acc declare create (YYTTND)
!$acc declare create (YYTGMVT95)
!$acc declare create (YYTGFLT95)
