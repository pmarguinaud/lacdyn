...
CALL SITNU(YRVFE,YRVETA,YRDYN,YRDIMV,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,&
          &PGMV(:,:,YDGMV%YT0%MDIV),PTOD0,PSDIV0)
...
CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0L,&
          &PGMV(:,:,YDGMV%YT0%MTL),PGMVS(:,YDGMV%YT0%MSPL))
CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0M,&
          &PGMV(:,:,YDGMV%YT0%MTM),PGMVS(:,YDGMV%YT0%MSPM))
...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    CALL GPRCP(YGFL,KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,PGFL=PGFL,PR=ZR9)  
    ...
    ENDDO
  ELSE
    CALL GPRCP(YGFL,KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JLON=KIDIA,KFDIA
        PTOD0(JLON,JLEV)=RD*PTOD0(JLON,JLEV)/ZR9(JLON,JLEV)
