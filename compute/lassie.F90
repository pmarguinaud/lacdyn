...
CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0L,&
          &PGMV(1,1,YDGMV%YT0%MTL),PGMVS(1,YDGMV%YT0%MSPL),ISTPT,KSTSZ,PSTACK)
CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT0M,&
          &PGMV(1,1,YDGMV%YT0%MTM),PGMVS(1,YDGMV%YT0%MSPM),ISTPT,KSTSZ,PSTACK)
...
IF (.NOT.LTWOTL) THEN
  ...
  !   and Tau*D (SI term for temperature equation).
  CALL SITNU(YRVFE,YRVETA,YRDYN,YRDIMV,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,&
           &PGMV(1,1,YDGMV%YT9%MDIV),PTOD9,PSDIV9,ISTPT,KSTSZ,PSTACK)
  ! - Computation of Nabla(Gamma*T+Mu*Pi) (SI term for momentum equation).
  CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9L,&
            &PGMV(1,1,YDGMV%YT9%MTL),PGMVS(1,YDGMV%YT9%MSPL),ISTPT,KSTSZ,PSTACK)
  CALL SIGAM(YRVFE,YRVETA,YRDYN,KIDIA,KFDIA,KLON,YRDIMV%NFLEVG,PGAGT9M,&
            &PGMV(1,1,YDGMV%YT9%MTM),PGMVS(1,YDGMV%YT9%MSPM),ISTPT,KSTSZ,PSTACK)
ENDIF
...
IF (LSPRT) THEN
  IF (LTWOTL) THEN
    ...
    CALL GPRCP(YGFL,KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,ISTPT,KSTSZ,PSTACK,PGFL=PGFL,PR=ZR9)  
    ...
    ENDDO
  ELSE
    CALL GPRCP(YGFL,KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,ISTPT,KSTSZ,PSTACK,PGFL=PGFL,KGFLTYP=9,PR=ZR9)  
    DO JLEV=1,YRDIMV%NFLEVG
      DO JLON=KIDIA,KFDIA
        PTOD0(JLON,JLEV)=RD*PTOD0(JLON,JLEV)/ZR9(JLON,JLEV)
