...
temp (REAL(KIND=JPRB), ZREDIV, (KLON))
! * computed in LASSIE or LANHSI:

temp (REAL(KIND=JPRB), ZBDT, (KLON))

temp (REAL(KIND=JPRB), ZGAGT0M, (KLON,YRDIMV%NFLEVG))
temp (REAL(KIND=JPRB), ZGAGT0L, (KLON,YRDIMV%NFLEVG))

temp (REAL(KIND=JPRB), ZGAGT9M, (KLON,YRDIMV%NFLEVG))
temp (REAL(KIND=JPRB), ZGAGT9L, (KLON,YRDIMV%NFLEVG))

temp (REAL(KIND=JPRB), ZSDIV9, (KLON))
temp (REAL(KIND=JPRB), ZSDIV0, (KLON))

temp (REAL(KIND=JPRB), ZTOD9, (KLON,YRDIMV%NFLEVG))
temp (REAL(KIND=JPRB), ZTOD0, (KLON,YRDIMV%NFLEVG))
...
CALL LASURE(YRPHY,YRGEM,YRDYN,YRDIM,KLON,KIDIA,KFDIA,PBETADT,PDT,YDGSGEOM,&
 & ZDTS2,ZBT,LL2TLFF1,ZBDT,ZREDIV,ZESGP,ZESGM,ISTPT,KSTSZ,PSTACK)  
...
CALL LASSIE(YRVFE,YRVETA,YGFL,YRDYN,YRDIMV,YRDIM,KLON,YDGMV,KIDIA,KFDIA,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
& ZSDIV0,ZSDIV9,ZTOD0,ZTOD9,ZGAGT0L,ZGAGT0M,ZGAGT9L,ZGAGT9M,ISTPT,KSTSZ,PSTACK)  
...
IF ((YRDYN%NCURRENT_ITER == 0).OR. (&
 & YRDYN%NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  ...
  CALL LAVENT(YRVETA,YRPARAR,YRDYN,YRDIMV,YRDIM,&
            & KLON,YDGMV,KIDIA,KFDIA,KSETTLOFF,LL2TLFF1,PVCRS0,PRDLR0,ZDTS2,PRDELP0,&
            & PCTY0(1,0,YYTCTY0%M_EVEL),PATND,PGMV,PB1,PB2,PWRL95,ISTPT,KSTSZ,PSTACK)
...
CALL LATTEX(YGFL,YRVAB,YRSTA,YRMDDH,YRLDDH,YRDYN,YRDIMV,YRDIM,&
 & KLON,YDGMV,KIDIA,KFDIA,ZDTS2,ZBT,ZBDT,ZESGP,ZESGM,&
 & YDGSGEOM,YDOROG,POROGL,POROGM,&
 & PVCRS0,PVCRSSA9F,PRDLR0,PRDLR9,PVCW0F,PWDLW0F,&
 & ZGAGT0L,ZGAGT0M,ZTOD0,&
 & ZGAGT9L,ZGAGT9M,ZTOD9,&
 & PRDELP0,PCTY0(1,0,YYTCTY0%M_EVEL),PATND,&
 & PNHXT0,PGWT0,PNHXT9,PGWT9,PGFL,&
 & PGMV,PGMVT1,PGMVTNDSI,PB1,PB2,ISTPT,KSTSZ,PSTACK)
...
CALL LATTES(YRVFE, YRVETA, YRVAB, YRSTA, YRDYN, YRDIMV, YRDIM,&
 & KLON,YDGMV,KIDIA,KFDIA,ZDTS2,ZBDT,ZESGP,ZESGM,&
 & PVCRS0,PRDLR0,YDOROG,POROGL,POROGM,&
 & ZSDIV0,ZSDIV9,PCTY0(1,0,YYTCTY0%M_PSDVBC),PRES0,PGMVS,&
 & PGMV,PGMVT1S,PB1,PB2,ISTPT,KSTSZ,PSTACK)
...
IF ((YRDYN%NCURRENT_ITER == 0).OR. (&
 & YRDYN%NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  CALL LAVABO(YGFL,YRDYN,YRDIMV,YRDIM,KLON,KIDIA,KFDIA,LL2TLFF1,PB1,ISTPT,KSTSZ,PSTACK)
ENDIF
