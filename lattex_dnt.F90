...
REAL(KIND=JPRB) :: ZXNLT1(YRDIM%NPROMA)
REAL(KIND=JPRB) :: ZXNLT0(YRDIM%NPROMA)
...
IF (YRLDDH%LRSIDDH) THEN
  IF ( .NOT.LLCT ) THEN
    IF (KXLAG >= 3) THEN
      IF (NSTEP <= 0 .OR. LLNESC) THEN
        PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG) &
         & +PESGP*ZXIDT0*PXSI(KST:KPROF,1:YRDIMV%NFLEVG)
      ELSEIF (LDSETTLS) THEN
        PSIDDHXL0(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXL0(KST:KPROF,1:YRDIMV%NFLEVG) &
         & +(ZXIDT0*PXSI(KST:KPROF,1:YRDIMV%NFLEVG)-PSIDDHXT9(KST:KPROF,1:YRDIMV%NFLEVG))
        PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG) &
         &+PESGP*ZXIDT0*PXSI(KST:KPROF,1:YRDIMV%NFLEVG)
      ELSE
        ! * remaining case: ldsettls=false, lnesc=false.
        PSIDDHXL0(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXL0(KST:KPROF,1:YRDIMV%NFLEVG) &
         & +0.5_JPRB*PESGM*(ZXIDT0*PXSI(KST:KPROF,1:YRDIMV%NFLEVG) &
         & -PSIDDHXT9(KST:KPROF,1:YRDIMV%NFLEVG))
        PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG) &
         & +PESGP*(1.5_JPRB*ZXIDT0*PXSI(KST:KPROF,1:YRDIMV%NFLEVG) &
         & -0.5_JPRB*PSIDDHXT9(KST:KPROF,1:YRDIMV%NFLEVG))
      ENDIF
    ENDIF
    IF ( .NOT.LLNESC ) THEN
      ! save of semi-implicit linear term  at time t
      ! to be used at time t-dt next time step
      PSIDDHXT9(KST:KPROF,1:YRDIMV%NFLEVG)=ZXIDT9*PXSI(KST:KPROF,1:YRDIMV%NFLEVG)
    ENDIF
  ELSE
    IF (KXLAG >= 3) THEN
      PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG)=PSIDDHXT1(KST:KPROF,1:YRDIMV%NFLEVG) &
       & +PESGP*ZXIDT9*PXSI(KST:KPROF,1:YRDIMV%NFLEVG)
    ENDIF
  ENDIF
ENDIF
