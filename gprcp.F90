...
!     Vectorizes:
      ...
      ZR(KSTART:KPROF,:) = ZGFL_R(1)*PGFL(KSTART:KPROF,:,IPT(1))
      DO JGFL=2,INUMACT
        CALL ACCUMULATE(ZGFL_R(JGFL),PGFL(1,1,IPT(JGFL)),ZR)
      ENDDO
      ZR(KSTART:KPROF,:) = RD+ZR(KSTART:KPROF,:)
    ENDIF
  ENDIF
  IF(LLCP) THEN
    IF(INUMACT == 0) THEN
...
!     Vectorizes:
      ...
      ZCP(KSTART:KPROF,:) = ZGFL_CP(1)*PGFL(KSTART:KPROF,:,IPT(1))
      DO JGFL=2,INUMACT
        CALL ACCUMULATE(ZGFL_CP(JGFL),PGFL(1,1,IPT(JGFL)),ZCP)
      ENDDO
      ZCP(KSTART:KPROF,:) = RCPD+ZCP(KSTART:KPROF,:)
    ENDIF
  ENDIF
ELSE
  IF(LLR) THEN
...
! * fill PR,PCP,PKAP:
IF (LLR) PR(KSTART:KPROF,:)=ZR(KSTART:KPROF,:)
IF (LLCP) PCP(KSTART:KPROF,:)=ZCP(KSTART:KPROF,:)
IF (LLKAP) PKAP(KSTART:KPROF,:) = ZR(KSTART:KPROF,:)/ZCP(KSTART:KPROF,:)
