...
! * [vertical weight]_l = [delta B]_l
  DO JLEV=1,YRDIMV%NFLEVG
    ZVWEI(KIDIA:KFDIA,JLEV)=YRVAB%VDELB(JLEV)
  ENDDO
ENDIF
  ...
  IF(.NOT.LLCT)THEN
    ...
    DO JLEV=1,YRDIMV%NFLEVG
      ...
      IF( LPC_FULL )THEN
        ...
        PGMV(KIDIA:KFDIA,JLEV,YDGMV%YT9%MCSPNL)=ZMOY1SP(KIDIA:KFDIA,JLEV)
      ENDIF
      ...
      IF( .NOT.LNESC )THEN
        ...
        IF (LSLINLC1) THEN
          PGMV(KIDIA:KFDIA,JLEV,YDGMV%YT9%MSPNL)=ZMOY1SP(KIDIA:KFDIA,JLEV)
          PGMV(KIDIA:KFDIA,JLEV,YDGMV%YT9%MSPNL_SI)=ZXIDT9*PB2(KIDIA:KFDIA,YRPTRSLB2%MSLB2SPSI)
        ELSE
          DO JLON=KIDIA,KFDIA
      ...
      IF(YRDYN%NVLAG == 2 .OR. YRDYN%NVLAG == 3)THEN
        IF( LVFE_INTB )THEN
          DO JLON=KIDIA,KFDIA
            ...
            ZIN2(JLON,1:YRDIMV%NFLEVG)=ZSPNLT_FE(JLON,1:YRDIMV%NFLEVG)
...
!         CALL VERINT(KLON,KIDIA,KFDIA,YRDIMV%NFLEVG,YRDIMV%NFLEVG+1,YRVFE%RINTE,ZSPNLT_FE,ZSPT1,0)
  ...
  ELSE
    ...
    DO JLEV=1,YRDIMV%NFLEVG
      ...
      IF (LSLINLC1) THEN
        ZSPNLT1(KIDIA:KFDIA)=ZMOY1SP(KIDIA:KFDIA,JLEV)
        ZSPNLT1_SI(KIDIA:KFDIA)=ZXIDT9*PB2(KIDIA:KFDIA,YRPTRSLB2%MSLB2SPSI)
      ELSE
        DO JLON=KIDIA,KFDIA
