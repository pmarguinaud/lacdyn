...
INTEGER(KIND=JPIM) :: IPSTPT_ZMOY1T

INTEGER(KIND=JPIM) :: IPSTPT_ZMOY1V

INTEGER(KIND=JPIM) :: IPSTPT_ZMOY1U

INTEGER(KIND=JPIM) :: IPSTPT_ZWT0
...
#define ZWT0(i) PSTACK (i,IPSTPT_ZWT0)

IPSTPT_ZMOY1U = IPSTPT

IPSTPT = IPSTPT + YRDIMV%NFLEVG

#define ZMOY1U(i,j) PSTACK (i,IPSTPT_ZMOY1U+j-(1))

IPSTPT_ZMOY1V = IPSTPT

IPSTPT = IPSTPT + YRDIMV%NFLEVG

#define ZMOY1V(i,j) PSTACK (i,IPSTPT_ZMOY1V+j-(1))

IPSTPT_ZMOY1T = IPSTPT

IPSTPT = IPSTPT + YRDIMV%NFLEVG

#define ZMOY1T(i,j) PSTACK (i,IPSTPT_ZMOY1T+j-(1))
...
IF (LTWOTL) THEN
  ...
  CALL LATTEX_DNT(PSTACK, KPSTSZ, IPSTPT, KLON,KIDIA,KFDIA,LLSETTLSW,YRDYN%NWLAG,PESGP,PESGM,&
   & PGMV(1,1,YDGMV%YT0%MU),PGMV(1,1,YDGMV%YT9%MU),ZMOY1U(1,1),&
  ...
  CALL LATTEX_DNT(PSTACK, KPSTSZ, IPSTPT, KLON,KIDIA,KFDIA,LLSETTLSW,YRDYN%NWLAG,PESGP,PESGM,&
   & PGMV(1,1,YDGMV%YT0%MV),PGMV(1,1,YDGMV%YT9%MV),ZMOY1V(1,1), &
...
IF (LTWOTL) THEN
  ...
  CALL LATTEX_DNT(PSTACK, KPSTSZ, IPSTPT, KLON,KIDIA,KFDIA,LSETTLS,YRDYN%NTLAG,PESGP,PESGM, &
   & PGMV(1,1,YDGMV%YT0%MT),PGMV(1,1,YDGMV%YT9%MT),ZMOY1T(1,1),&
