...
KSTPT = 1
KSTSZ = KLON * YRDIMV%NFLEVG * 20
ALLOCATE (PSTACK_ALL (KSTSZ, ICOUNT))

PRINT *, "-- RUN"

!$OMP PARALLEL DO PRIVATE (JJ,IBL,JIDIA,JFDIA)
DO IBL = 1, ICOUNT
...
PRINT *, IBL
  ...
  DO JJ = KIDIA, KFDIA
    ...
    CALL LACDYN(KLON,YDGMV_ALL(IBL),JIDIA,JFDIA,PBETADT,PDT,&
     ...
     & PGMVTNDSI_ALL(:,:,:,IBL),PWRL95_ALL(:,:,IBL),KSTPT,KSTSZ,PSTACK_ALL(:,IBL))
