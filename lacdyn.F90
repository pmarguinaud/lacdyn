SUBROUTINE LACDYN(PSTACK, KPSTSZ, KLON, YDGMV, &
 ! --- INPUT ------------------------------------------------------------------
...
CALL LASURE(PSTACK, KPSTSZ, KLON,KIDIA,KFDIA,PBETADT,PDT,YDGSGEOM,&
...
CALL LASSIE(PSTACK, KPSTSZ, KLON,YDGMV,KIDIA,KFDIA,YDGSGEOM%RCORI,PGMV,PGMVS,PGFL,&
...
IF ((YRDYN%NCURRENT_ITER == 0).OR. (&
 & YRDYN%NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  ...
  CALL LAVENT(PSTACK, KPSTSZ, KLON,YDGMV,KIDIA,KFDIA,KSETTLOFF,LL2TLFF1,PVCRS0,PRDLR0,ZDTS2,PRDELP0,&
...
CALL LATTEX(PSTACK, KPSTSZ, KLON,YDGMV,KIDIA,KFDIA,ZDTS2,ZBT,ZBDT,ZESGP,ZESGM,&
 & YDGSGEOM,YDOROG,POROGL,POROGM,&
...
CALL LATTES(PSTACK, KPSTSZ, KLON,YDGMV,KIDIA,KFDIA,ZDTS2,ZBDT,ZESGP,ZESGM,&
...
IF ((YRDYN%NCURRENT_ITER == 0).OR. (&
 & YRDYN%NCURRENT_ITER > 0 .AND. LPC_FULL .AND.(.NOT.LPC_CHEAP) )) THEN
  CALL LAVABO(PSTACK, KPSTSZ, KLON,KIDIA,KFDIA,LL2TLFF1,PB1)
ENDIF
